from lxml import etree
from xmldiff import main as xmldiff_main, patch
import os


def user_input_dir_to_files():
    print("Vložte absolútnu cestu k priečinku so súbormi")

    while True:
        input_path = input()

        if not input_path or input_path.strip() == "":
            print("Zadajte platnú cestu (nie prázdnu). Skúste znova:")
            continue

        input_path = input_path.strip().strip('"')
        full_path = os.path.abspath(input_path)

        if not os.path.isdir(full_path):
            print(f"Adresár neexistuje: {full_path}. Skontrolujte cestu a skúste znova:")
            continue

        return full_path


def load_xml(full_path):
    return etree.parse(full_path)


def save_xml(tree, full_path):
    tree.write(full_path, pretty_print=True, encoding="utf-8", xml_declaration=True)


def write_iteration_scripts(scripts_path, iteration, diff_left, diff_right):
    # Súbor sa priebežne dopĺňa, aby zostal zachovaný výstup zo všetkých iterácií.
    with open(scripts_path, "a", encoding="utf-8") as file:
        file.write(f"Iterácia {iteration}\n")
        file.write("LEFT script:\n")
        if diff_left:
            for action in diff_left:
                file.write(f"{action}\n")
        else:
            file.write("(bez zmien)\n")

        file.write("RIGHT script:\n")
        if diff_right:
            for action in diff_right:
                file.write(f"{action}\n")
        else:
            file.write("(bez zmien)\n")

        file.write("\n")


def _short_text(text, max_len=80):
    normalized = " ".join((text or "").split())
    if len(normalized) <= max_len:
        return normalized
    return normalized[:max_len - 3] + "..."


def _append_xml_structure(lines, node, depth=0, path=None):
    if not isinstance(node.tag, str):
        return

    current_path = path or f"/{node.tag}"
    element_children = [child for child in node if isinstance(child.tag, str)]

    parts = [f"{'  ' * depth}- {current_path}", f"tag={node.tag}", f"children={len(element_children)}"]

    if node.attrib:
        attrs = ", ".join(f"{key}={value}" for key, value in sorted(node.attrib.items()))
        parts.append(f"attrs=[{attrs}]")

    text_value = _short_text(node.text)
    if text_value:
        parts.append(f"text=\"{text_value}\"")

    lines.append(" | ".join(parts))

    child_tag_counts = {}
    for child in node:
        if not isinstance(child.tag, str):
            continue

        child_tag_counts[child.tag] = child_tag_counts.get(child.tag, 0) + 1
        child_path = f"{current_path}/{child.tag}[{child_tag_counts[child.tag]}]"
        _append_xml_structure(lines, child, depth + 1, child_path)


def xml_tree_structure_text(tree_or_root):
    # Umožní prijať etree.ElementTree aj priamo koreňový etree.Element.
    root = tree_or_root.getroot() if hasattr(tree_or_root, "getroot") else tree_or_root
    lines = []
    _append_xml_structure(lines, root)
    return "\n".join(lines)


def write_iteration_tree_dump(trees_path, iteration, base_tree, left_tree, right_tree, merged_root):
    # Slúži na audit priebehu merge: uloží štruktúru všetkých stromov v danej iterácii.
    with open(trees_path, "a", encoding="utf-8") as file:
        file.write(f"Iterácia {iteration}\n")
        file.write("BASE tree:\n")
        file.write(xml_tree_structure_text(base_tree) + "\n")
        file.write("LEFT tree:\n")
        file.write(xml_tree_structure_text(left_tree) + "\n")
        file.write("RIGHT tree:\n")
        file.write(xml_tree_structure_text(right_tree) + "\n")
        file.write("MERGED tree:\n")
        file.write(xml_tree_structure_text(merged_root) + "\n\n")


def merge_three_way(base_path, left_path, right_path, result_path, scripts_path=None, iteration=None):
    base_tree = load_xml(base_path)
    left_tree = load_xml(left_path)
    right_tree = load_xml(right_path)

    # Získame edit scripty voči spoločnej BASE verzii.
    diff_left = xmldiff_main.diff_trees(base_tree, left_tree)
    diff_right = xmldiff_main.diff_trees(base_tree, right_tree)

    if scripts_path is not None and iteration is not None:
        write_iteration_scripts(scripts_path, iteration, diff_left, diff_right)

    patcher = patch.Patcher()
    merge_errors = []
    try:
        # Preferované poradie: najprv LEFT zmeny, potom RIGHT zmeny.
        merged_root = patcher.patch(diff_left, base_tree)
        merged_root = patcher.patch(diff_right, merged_root)
    except Exception as first_ex:
        merge_errors.append(("LEFT->RIGHT", str(first_ex)))
        # Pri konflikte skúšame opačné poradie aplikovania patchov.
        try:
            merged_root = patcher.patch(diff_right, base_tree)
            merged_root = patcher.patch(diff_left, merged_root)
        except Exception as second_ex:
            merge_errors.append(("RIGHT->LEFT", str(second_ex)))
            merge_error = RuntimeError(
                "Nepodarilo sa aplikovať patch ani v jednom poradí. "
                f"LEFT->RIGHT chyba: {first_ex}; RIGHT->LEFT chyba: {second_ex}"
            )
            merge_error.merge_errors = merge_errors
            raise merge_error from second_ex

    if iteration is not None:
        trees_path = os.path.join(os.path.dirname(result_path), "merged_trees.txt")
        write_iteration_tree_dump(
            trees_path,
            iteration,
            base_tree,
            left_tree,
            right_tree,
            merged_root,
        )

    save_xml(etree.ElementTree(merged_root), result_path)
    return merge_errors


def main():
    DirWithFiles = user_input_dir_to_files()
    result_dir = os.path.join(DirWithFiles, "xmldiff")
    if not os.path.exists(result_dir):
        os.mkdir(result_dir)

    scripts_path = os.path.join(result_dir, "scripts.txt")
    with open(scripts_path, "w", encoding="utf-8") as file:
        file.write("Edit scripty podľa iterácií\n\n")

    iteration = 0
    errored_files = []
    errored_details = []

    while True:

        # Očakávaná štruktúra vstupu: /<koreň>/<iterácia>/(base|left|right)<iterácia>.xml
        base_path = os.path.join(DirWithFiles, str(iteration), f"base{iteration}.xml")
        left_path = os.path.join(DirWithFiles, str(iteration), f"left{iteration}.xml")
        right_path = os.path.join(DirWithFiles, str(iteration), f"right{iteration}.xml")
        result_path = os.path.join(result_dir, f"mergedResult{iteration}.xml")

        if not (os.path.isfile(base_path) and
                os.path.isfile(left_path) and
                os.path.isfile(right_path)):

            # Koniec dávky: iterácia bez kompletnej trojice vstupných súborov.

            print(f"Ukončené na iterácii: {iteration}")

            if iteration == 0:
                print("Nenájdený žiadny súbor na spracovanie.")
            else:
                print(f"Počet chybne spracovaných súborov: {len(errored_files)}")

                if errored_files:
                    print("Chybné iterácie:", ", ".join(map(str, errored_files)))

                conflict_percent = (len(errored_files) * 100.0) / iteration
                print(f"{conflict_percent:.2f}% chybne spracovaných iterací.")

            if len(errored_details) > 0:
                error_file_path = os.path.join(result_dir, "xmlDiffErrors.txt")
                with open(error_file_path, "w", encoding="utf-8") as f:
                    errors_by_iteration = {}
                    for item_iteration, item_order, item_error in errored_details:
                        if item_iteration not in errors_by_iteration:
                            errors_by_iteration[item_iteration] = {
                                "LEFT->RIGHT": [],
                                "RIGHT->LEFT": [],
                            }

                        if item_order in errors_by_iteration[item_iteration]:
                            errors_by_iteration[item_iteration][item_order].append(item_error)

                    for item_iteration in sorted(errors_by_iteration):
                        left_errors = errors_by_iteration[item_iteration]["LEFT->RIGHT"]
                        right_errors = errors_by_iteration[item_iteration]["RIGHT->LEFT"]

                        f.write(f"Iterácia {item_iteration}:\n")
                        f.write("Left->right:\n")
                        if left_errors:
                            for item_error in left_errors:
                                f.write(f"{item_error}\n")
                        else:
                            f.write("(bez chyby)\n")

                        f.write("Right->Left:\n")
                        if right_errors:
                            for item_error in right_errors:
                                f.write(f"{item_error}\n")
                        else:
                            f.write("(bez chyby)\n")

                        f.write("\n")

                print(f"Zoznam chýb uložený do: {error_file_path}")

            break

        try:
            iteration_merge_errors = merge_three_way(
                base_path,
                left_path,
                right_path,
                result_path,
                scripts_path=scripts_path,
                iteration=iteration,
            )
            for item_order, item_error in iteration_merge_errors:
                errored_details.append((iteration, item_order, item_error))
            print(f"Iterácia {iteration} spojená úspešne.")
        except Exception as ex:
            print(f"Chyba pri mergovaní v iterácii {iteration}: {ex}")
            errored_files.append(iteration)
            exception_merge_errors = getattr(ex, "merge_errors", None)
            if exception_merge_errors:
                for item_order, item_error in exception_merge_errors:
                    errored_details.append((iteration, item_order, item_error))
            else:
                errored_details.append((iteration, "UNKNOWN", str(ex)))

        iteration += 1


if __name__ == "__main__":
    main()
